...
const Products = () => {
  ...
  const performAPICall = async () => {
    ...
    setIsLoading(false);
    if (error.response && error.response.status === 500) { // 500 -> internal server error mawns backend failed
  ...
  const performSearch = async (text) => {
    try{
      ...
      if (error.response) { // 500 -> internal server error mawns backend failed
  ...
  const fetchCart = async (token) => {
    ...
    try {
      ...
      {
        headers : {
          Authorization  : `Bearer ${token}`,// authetication is during login but after login authorization is checked to confirm
        }
      });
      return response.data;
    } catch (e) {
         ...
         <Grid item className="product-grid" xs={12} md={token && products.length ? 9 : 12}>
           ...
           </Box>
           {
            ...
            <Box className="loading">
              ...
              <Grid container marginY="1rem" marginX="1rem" spacing={{ xs: 2, md: 3 ,lg:1 }}>
              {
                filteredProducts.length ? (
                  filteredProducts.map((filteredProduct) => (
                    <Grid item xs={6} md={3} key={filteredProduct._id} >
                     <ProductCard product={filteredProduct} handleAddToCart={async () =>{
                      await addToCart(
                        token,
                        items,
                        products,
          ...
          {token ?
            (
              <Grid item xs={12} md={3} sx={{ bgcolor: "#E9F5E1" }} >
                <Cart products={products} items={items} handleQuantity={addToCart} hasCheckoutButton/>
              </Grid>
            ) :
            null}
          </Grid>
